...
class Simulation():
    def __init__(self, Model, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_states, num_actions, training_epochs):
        ...
        self._incoming_density_store = []
        self._outgoing_density_store = []
        self._pressure_store = []
    ...
    def run(self, episode, epsilon):
        ...
        self._sum_speed = 0
        self._incoming_density = 0
        self._outgoing_density = 0
        self._pressure = 0
        old_total_wait = 0
        old_state = -1
        old_action = -1
    ...
    def _simulate(self, steps_todo):
        ...
        while steps_todo > 0:
            ...
            total_speed = self._get_speed()
            self._sum_speed += total_speed

            incoming_density, outgoing_density, pressure = self._get_density_and_pressure()
            self._incoming_density += incoming_density
            self._outgoing_density += outgoing_density
    ...
    def _choose_action(self, state, epsilon):
        ...
        if random.random() < epsilon:
            ...
            return np.argmax(self._Model.predict_one(state).cpu().numpy()) # the best action given the current state
    ...
    def _replay(self):
        ...
        if len(batch) > 0:  # if the memory is full enough
            ...
            for i, b in enumerate(batch):
                ...
                x[i] = state
                y[i] = current_q.cpu().numpy()  # Q(state) that includes the updated action value
    ...
    def _save_episode_stats(self):
        ...
        self._avg_speed_store.append(self._sum_speed / self._max_steps)
        self._incoming_density_store.append(self._incoming_density)
        self._outgoing_density_store.append(self._outgoing_density)
        self._pressure_store.append(self._pressure)
    ...
    @property
    def incoming_density_store(self):
        return self._incoming_density_store

    @property
    def outgoing_density_store(self):
        return self._outgoing_density_store

    @property
    def pressure_store(self):
        return self._pressure_store
